...
"""
        ...
        fallback_models = ["models/gemini-2.5-flash", "models/gemini-2.5-flash-lite", "models/gemini-1.5-flash", "models/gemini-flash-latest"]
        for fm in fallback_models:
            try:
                model = genai.GenerativeModel(fm)
                return model.generate_content(full_instruction).text + "\n\n(注意: Google Searchがクォータ制限のため、AIの知識のみで回答しました)"
            except:
                continue
        
        raise Exception(f"全構成で失敗しました。最新エラー: {last_error}\n(無料枠の1日あたりのリクエスト上限に達した可能性があります)")
